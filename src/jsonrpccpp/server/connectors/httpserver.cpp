...
// are deprecated.
    ...
    if (this->bindlocalhost) {
      ...
      try {
        ...
        this->daemon = MHD_start_daemon(MHD_USE_TLS | MHD_USE_ERROR_LOG | mhd_flags, this->port, NULL, NULL, HttpServer::callback, this, MHD_OPTION_HTTPS_MEM_KEY,
                                        this->sslkey.c_str(), MHD_OPTION_HTTPS_MEM_CERT, this->sslcert.c_str(), MHD_OPTION_THREAD_POOL_SIZE, this->threads,
                                        MHD_OPTION_END); // bug here.
      } catch (JsonRpcException &ex) {
        std::cout << "JsonRpcException/" << ex.what() << "\n";
        return false;
      }
    } else {
      this->daemon =
